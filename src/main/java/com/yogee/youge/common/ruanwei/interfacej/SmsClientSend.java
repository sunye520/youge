package com.yogee.youge.common.ruanwei.interfacej;

import java.net.URLEncoder;

import com.yogee.youge.common.ruanwei.tool.SmsClientAccessTool;
import com.yogee.youge.common.utils.DateUtils;
import com.yogee.youge.common.weixin.kit.EncryptionKit;
...
public class SmsClientSend {
	...
	public static String sendSms(String url, String userid, String account,
		...
		String timestamp = DateUtils.getDateTimestamp();//20120701231212
		String sign = account+password+timestamp;
		String signMd5 = EncryptionKit.md5Encrypt(sign).toLowerCase();
		String sendTime = "";//为空表示立即发送，定时发送格式2010-10-24 09:08:10
		String action = "send";

//		return "";
		return sendSms(url, userid, account, password, mobile, content, null,
				sendTime, null, null, null, null, null, "POST", "UTF-8", "UTF-8", timestamp, signMd5);
	}
	...
	private static String sendSms(String url, String userid, String account,
								  ...
								  String sendType, String codingType, String backEncodType, String timestamp, String sign) {
		...
		try {
			...
			if (sign != null && !sign.equals("")) {
				send.append("&sign=").append(
						URLEncoder.encode(sign, codingType));
			}
			if (timestamp != null && !timestamp.equals("")) {
				send.append("&timestamp=").append(
						URLEncoder.encode(timestamp, codingType));
			}
